...
OAuth 2.0 (see [OAuth 2.0 Authentication](/v1.13.x/how-to-guides/mcp/oauth)) requires a person to sign in through a browser, so it doesn't fit an unattended agent. Bots also can't generate a Personal Access Token — OpenMetadata rejects that request. Use a Bot's JSON Web Token (JWT) instead:
...
2. On the Bot's details page, click **Generate new token** to issue its JWT, following [Bot Token](/v1.13.x/how-to-guides/mcp#bot-token).
...
See the [MCP Server Connection Guide](/v1.13.x/how-to-guides/mcp/connect) for the full protocol reference, including headers, `tools/list`, and prompts. See the [MCP Tools Reference](/v1.13.x/how-to-guides/mcp/reference) for every available tool.
