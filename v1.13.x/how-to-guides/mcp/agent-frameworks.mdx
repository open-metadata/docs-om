...
OAuth 2.0 (see [OAuth 2.0 Authentication](/v1.13.x/how-to-guides/mcp/oauth)) requires a person to sign in through a browser, so it doesn't fit an unattended agent. Use a Personal Access Token generated from a Bot instead:

1. Follow [How to Set Up Bots](/v1.13.x/developers/bots) to create a Bot and assign it the role-based access policy your agent needs.
2. Generate a Personal Access Token for that Bot, following [Personal Access Token (PAT)](/v1.13.x/how-to-guides/mcp#personal-access-token-pat).
...
See the [MCP Server Connection Guide](/v1.13.x/how-to-guides/mcp/connect) for the full protocol reference (headers, `tools/list`, prompts) and the [MCP Tools Reference](/v1.13.x/how-to-guides/mcp/reference) for every available tool.
...
Since there's no human watching for a stuck spinner, a headless caller must explicitly check `result.isError` on every tool call rather than assuming a 200-shaped response succeeded. See [Tool Execution Errors](/v1.13.x/how-to-guides/mcp/connect-api#tool-execution-errors) for the full envelope shape and a worked example.

## Next Steps

- [MCP Tools Reference](/v1.13.x/how-to-guides/mcp/reference) for the full tool catalog.
- [MCP Server Connection Guide](/v1.13.x/how-to-guides/mcp/connect) for the complete protocol reference.
