...
**If you already have a fully qualified name**, start with `get_entity_details` directly. **If you don't**, start with `search_metadata` or `semantic_search` first, then pass the `fullyQualifiedName` from a result into `get_entity_details`.
...
Consult the entity's [API Reference](/v1.13.x/api-reference/data-assets) page for entity types not listed here.
