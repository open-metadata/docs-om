...
* Secret Key: Pass the JSON file generated in [Create Service Account](/v1.13.x/deployment/security/google) as the secretKey.
...
* Private Key: Use the Public/Private Key Pair that was generated while [Creating the Service Application](/v1.13.x/deployment/security/okta#okta-sso). When copy-pasting the keys ensure that there are no additional codes and that it is a JSON compatible string.
...
Review the [OpenMetadata Java SDK examples](https://github.com/open-metadata/OpenMetadata/tree/1.13/openmetadata-sdk/src/main/java/org/openmetadata/sdk/examples) for more usage patterns.
