...
Where `x.y.z` is the version of the OpenMetadata ingestion package. Note that the version needs to match the server version. If we are using the server at 1.13.6, then the ingestion package needs to also be 1.13.6.

The plugin parameter is a list of the sources that we want to ingest. An example would look like this `openmetadata-ingestion[mysql,snowflake,s3]==1.13.6`.
...
with models.DAG(
    ...
    KubernetesPodOperator(
        ...
        name="ingest",
        cmds=["python", "main.py"],
        image="openmetadata/ingestion-base:1.13.6",
        namespace='default',
...
Note that the example uses the image `openmetadata/ingestion-base:1.13.6`. Update that accordingly for higher version
