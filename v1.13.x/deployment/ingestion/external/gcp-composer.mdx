...
It also requires an ingestion package from the matching server patch line. For OpenMetadata 1.13.1, install `openmetadata-ingestion~=1.13.1.0`.
...
- `openmetadata-ingestion[<plugins>]~=x.y.z.0`.
- `sqlalchemy==1.4.27`: This is needed to align OpenMetadata version with the Composer internal requirements.

Where `x.y.z.w` is the version of the OpenMetadata ingestion package. The first three segments need to match the server version. For OpenMetadata 1.13.1, use 1.13.1.0 or a later 1.13.1.x package release.
