...
It also requires the ingestion package to be at least `openmetadata-ingestion==1.13.5.0`.

## Using the Python Operator

<Warning>
This section's tested environment (Composer 2.5.4, Python 3.8) predates `openmetadata-ingestion` 1.13.5's requirements of Python 3.10 or newer and SQLAlchemy 2 or newer. Installing `openmetadata-ingestion==1.13.5.0` directly on that host, alongside the `sqlalchemy==1.4.27` pin below, won't resolve. If your Composer environment can't be upgraded to a Python 3.10+ image, use the [Kubernetes Pod Operator](#using-the-kubernetes-pod-operator) below instead. It runs the ingestion package inside the versioned `openmetadata/ingestion-base` container image, which ships the correct Python and SQLAlchemy versions regardless of the Composer host's own Python version.
</Warning>
...
Where `x.y.z` is the version of the OpenMetadata ingestion package. Note that the version needs to match the server version. If we are using the server at 1.13.5, then the ingestion package needs to also be 1.13.5.

The plugin parameter is a list of the sources that we want to ingest. An example would look like this `openmetadata-ingestion[mysql,snowflake,s3]==1.13.5`.
...
with models.DAG(
    ...
    KubernetesPodOperator(
        ...
        name="ingest",
        cmds=["python", "main.py"],
        image="openmetadata/ingestion-base:1.13.5",
        namespace='default',
...
Note that the example uses the image `openmetadata/ingestion-base:1.13.5`. Update that accordingly for higher version
