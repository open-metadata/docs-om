...
- **Image Version**: Ensure the Docker image version matches your OpenMetadata server version (e.g., `openmetadata/ingestion-base:1.13.5`).
...
with DAG(
    ...
    ingest_task = PythonVirtualenvOperator(
        ...
        requirements=[
            'openmetadata-ingestion[mysql]~=1.13.5',  # Specify any additional Python package dependencies
        ],
        system_site_packages=False,  # Set to True if you want to include system site-packages in the virtual environment
        python_version="3.10",  # Remove if necessary
