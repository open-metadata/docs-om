...
```
pip3 install "openmetadata-ingestion[&lt;plugin&gt;]~=x.y.z.0"
```
...
- Replace `x.y.z` with the OpenMetadata version matching your server. For example, use 1.13.1 for an OpenMetadata 1.13.1 server.

### Example
```
pip3 install "openmetadata-ingestion[mysql,snowflake,s3]~=1.13.1.0"
```
### Example DAG
