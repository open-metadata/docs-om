...
<CardGroup cols={1}>
<Card title="Lineage Workflow through Query Logs" href="/v1.13.x/connectors/ingestion/workflows/lineage/lineage-workflow-query-logs" >
...
From the Service Page, go to the **Agents** tab, click **Add Agent**, and select **Add Lineage Agent**.
...
In the [connectors](/v1.13.x/connectors) section we showcase how to run the metadata ingestion from a JSON/YAML file using the Airflow SDK or the CLI via metadata ingest. Running a lineage workflow is also possible using a JSON/YAML configuration file.
