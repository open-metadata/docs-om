...
- [Troubleshooting](/v1.13.x/connectors/pipeline/openlineage/troubleshooting)
...
Each dataset in an OpenLineage event is resolved using the following priority:

1. **Symlink identifiers** — if the event includes a `symlinks` facet, those identities are tried first. `LOCATION`-type symlinks (raw physical paths) are skipped. This is the shape Spark emits for Hive/Glue-cataloged tables backed by object storage (e.g. S3).
2. **Top-level namespace and name** — used as fallback if no symlink resolves.

The first candidate that matches a configured OpenMetadata service wins.

### Hive / Glue Tables on Object Storage

When Spark writes to a table in a Glue or Hive catalog backed by S3, the OpenLineage event identifies the dataset by its S3 path as the top-level namespace. The connector reads the `symlinks` facet first — which carries the catalog table identity — and resolves it correctly against the ingested service.
