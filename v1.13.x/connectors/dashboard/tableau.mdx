...
There is **no separate lineage agent or lineage pipeline** for Tableau — lineage is collected as part of the same metadata ingestion workflow. However, you do need to configure the **Database Service Prefixes** field (covered below) so OpenMetadata knows which database services to match against your Tableau dashboards.
...
In **Step 3 — Configure Metadata Ingestion** of the [Connection Details](#connection-details) section above:
