...
- [Lineage](#lineage)
- [Troubleshooting](/v1.13.x/connectors/dashboard/tableau/troubleshooting)
...
Lineage in OpenMetadata shows you which database tables power each Tableau dashboard. When lineage is set up, you can click on a dashboard and trace it back to the exact source tables in your database.

There is **no separate lineage agent** for Tableau. Lineage is collected automatically as part of the standard metadata ingestion workflow — no extra steps or pipelines needed.
...
- Follow the official Tableau guide to [enable the Tableau Metadata API](https://help.tableau.com/current/api/metadata_api/en-us/docs/meta_api_start.html#enable-the-tableau-metadata-api-for-tableau-server).

### How to Set Up Lineage

In **Step 3 — Configure Ingestion** of the [Connection Details](#connection-details) section above:

1. Scroll down to the **Lineage Information** section.
2. In the **Database Service Prefixes** field, enter the name of the database service in OpenMetadata that your Tableau dashboards connect to.

This tells OpenMetadata where to look for the source tables so it can draw the lineage path from table → dashboard.
...
<Tip>
If your Tableau dashboards pull data from multiple database services, add each service as a separate entry in the **Database Service Prefixes** field.
</Tip>
