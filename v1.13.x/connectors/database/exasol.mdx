...
- [Lineage](/v1.13.x/connectors/ingestion/workflows/lineage)
- [Troubleshooting](/v1.13.x/connectors/database/exasol/troubleshooting)
...
```
<Tip>
Use these broader privileges only when access to all schemas and tables is intended. `SELECT ANY TABLE` does not include system tables; the lineage workflow still requires `SELECT ANY DICTIONARY`.
</Tip>


### Lineage
For the lineage workflow, OpenMetadata fetches query logs by querying `EXA_STATISTICS.EXA_DBA_AUDIT_SQL`
and joining with `EXA_STATISTICS.EXA_DBA_AUDIT_SESSIONS`. To access these system tables, an
Exasol user must have the `SELECT ANY DICTIONARY` system privilege, and auditing must be
enabled on the database. For more information about Exasol auditing, see
