...
See the [Storage Configuration Overview](/v1.13.x/connectors/database/dbt/storage-configuration-overview) for setup guides:
- [AWS S3](/v1.13.x/connectors/database/dbt/storage-s3-guide) | [Google Cloud Storage](/v1.13.x/connectors/database/dbt/storage-gcs-guide) | [Azure Blob](/v1.13.x/connectors/database/dbt/storage-azure-guide) | [HTTP Server](/v1.13.x/connectors/database/dbt/storage-http-guide) | [Local/Shared Filesystem](/v1.13.x/connectors/database/dbt/storage-local-guide)
...
From the Service Page, go to the **Agents** tab, click **Add Agent**, and select **Add dbt Agent**.

<img src="/public/images/connector2.0/agents/add-dbt-agent.png" alt="Add dbt Agent" />

### 2. Configure the dbt Agent
...
Follow the link [here](/v1.13.x/connectors/database/dbt/setup-multiple-dbt-projects) for instructions on setting up multiple dbt projects.
...
To know how to get the values for `Dbt Cloud Account Id`, `Dbt Cloud Project Id` and `Dbt Cloud Job Id` fields check [here](/v1.13.x/connectors/database/dbt/run-dbt-workflow-externally).
