...
- [Metadata Ingestion](#metadata-ingestion)
    - [Incremental Extraction](/v1.13.x/connectors/ingestion/workflows/metadata/incremental-extraction/bigquery)
...
- [Troubleshooting](/v1.13.x/connectors/database/bigquery/troubleshooting)

<ExternalIngestionDeployment />
## Requirements
You need to create an service account in order to ingest metadata from bigquery refer [this](/v1.13.x/connectors/database/bigquery/create-credentials) guide on how to create service account.
<Columns cols={2}>
<Card title="Create Custom GCP Role" href="/v1.13.x/connectors/database/bigquery/create-credentials">
...
OpenMetadata supports lineage across multiple BigQuery projects without requiring a single service account to have cross-project access. Configure each BigQuery project as its own service, each with a service account scoped to that project only.

During lineage extraction, OpenMetadata resolves referenced tables against metadata already ingested — including tables from other BigQuery projects, as long as those projects are ingested as their own services. This applies to both view lineage and query lineage.

To include other projects when resolving lineage, enable **Process Cross Database Lineage** and set **Cross Database Service Names** to the names of the other BigQuery services when setting the lineage options during Agent Setup. Use `*` instead of listing names individually to search across all available services. For more information about these lineage options, see [Lineage Options](/v1.13.x/how-to-guides/data-lineage/workflow#lineage-options).
