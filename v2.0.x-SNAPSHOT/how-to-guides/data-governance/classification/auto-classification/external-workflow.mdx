...
| `sampleDataCount`             | Number of rows sampled per table for classification. These rows are always fetched; `storeSampleData` controls whether they are also saved to OpenMetadata. | Integer   | `50`                    |
...
- Rows are always fetched (up to `sampleDataCount`) for classification purposes regardless of this setting — `storeSampleData` only decides whether those rows are also saved to OpenMetadata.
- Defaults to `false`.
