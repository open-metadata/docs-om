...
* Secret Key: Pass the JSON file generated in [Create Service Account](/v2.0.x-SNAPSHOT/deployment/security/google) as the secretKey.
...
* Private Key: Use the Public/Private Key Pair that was generated while [Creating the Service Application](/v2.0.x-SNAPSHOT/deployment/security/okta#okta-sso). When copy-pasting the keys ensure that there are no additional codes and that it is a JSON compatible string.
...
* Authority: Use your Microsoft Entra tenant authority. See [Microsoft's OpenID Connect documentation](https://learn.microsoft.com/en-us/entra/identity-platform/v2-protocols-oidc) for the endpoint format.
...
azureSSOClientConfig.setScopes({scope_list});
```
Provide the Azure config while creating the [server connection](#establish-openmetadata-server-connection).
```java
...
Review the [OpenMetadata Java SDK examples](https://github.com/open-metadata/OpenMetadata/tree/main/openmetadata-sdk/src/main/java/org/openmetadata/sdk/examples) for more usage patterns.
