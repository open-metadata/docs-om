...
Semantic Search also powers the [Semantic Search MCP tool](/v2.0.x-SNAPSHOT/how-to-guides/mcp/semantic-search),
...
<Steps>
  ...
  <Step title="Embedding Generation & Vector Indexing">
    ...
    in a dedicated `vector_search_index` using the HNSW algorithm with cosine similarity. At query time,
...
- Check that the `vector_search_index` exists in your search backend.
