...
(e.g., [Athena](/v1.13.x-SNAPSHOT/connectors/database/athena/yaml)).
...
As explained in the [Managing Credentials](/v1.13.x-SNAPSHOT/deployment/ingestion/external/credentials#existing-services) guide, once a service exists in OpenMetadata its connection details are stored and can be reused — just omit the `serviceConnection` YAML entries in your DAG:
