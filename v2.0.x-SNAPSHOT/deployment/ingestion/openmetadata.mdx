...
<CardGroup cols={2}>
  ...
  <Card title="Kubernetes Native" href="/v2.0.x-SNAPSHOT/deployment/ingestion/kubernetes" icon="cube">
...
And then run the DAG as explained in each [Connector](/v2.0.x-SNAPSHOT/connectors), where `x.y.z` is the same version of your
...
You can check the [Connector Modules](/v2.0.x-SNAPSHOT/connectors) guide above to learn how to install the `openmetadata-ingestion` package with the
...
<Info>
For installation validation, Git Sync guidance, SSL configuration, and troubleshooting Airflow pipeline issues, see the [Airflow Troubleshooting & Advanced](/v2.0.x-SNAPSHOT/deployment/ingestion/openmetadata/troubleshooting) guide.
</Info>
