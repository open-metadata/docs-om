...
Let's start with a simplified design review of OpenMetadata. You can find further details [here](/v1.13.x-SNAPSHOT/main-concepts/high-level-design),
...
You can find all the necessary information on how to run the backups [here](/v1.13.x-SNAPSHOT/deployment/backup-restore-metadata).
...
- **Where**: We will restore the data to a clean database with the Flyway migrations at the same version as the backed up data.
  ...
  - We run the migrations ([docs](/v1.13.x-SNAPSHOT/deployment/bare-metal#4.-prepare-the-openmetadata-database-and-indexes))
     or we start the OpenMetadata server, which will automatically run the migrations.
  - With the server stopped, we [restore](/v1.13.x-SNAPSHOT/deployment/backup-restore-metadata) the data.
