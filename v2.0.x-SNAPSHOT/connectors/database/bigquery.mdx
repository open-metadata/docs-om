...
## Requirements
You need to create an service account in order to ingest metadata from bigquery refer [this](/v2.0.x-SNAPSHOT/connectors/database/bigquery/create-credentials) guide on how to create service account.
<Columns cols={2}>
...
To include other projects when resolving lineage, enable **Process Cross Database Lineage** and set **Cross Database Service Names** to the names of the other BigQuery services when setting the lineage options during Agent Setup. Use `*` instead of listing names individually to search across all available services. For more information about these lineage options, see [Lineage Options](/v2.0.x-SNAPSHOT/how-to-guides/data-lineage/workflow#lineage-options).
