...
<CardGroup cols={1}>
<Card title="Lineage Workflow through Query Logs" href="/v1.13.x-SNAPSHOT/connectors/ingestion/workflows/lineage/lineage-workflow-query-logs" >
...
In the [connectors](/v1.13.x-SNAPSHOT/connectors) section we showcase how to run the metadata ingestion from a JSON/YAML file using the Airflow SDK or the CLI via metadata ingest. Running a lineage workflow is also possible using a JSON/YAML configuration file.
