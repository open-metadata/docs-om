...
And then run the DAG as explained in each [Connector](/v1.11.x/connectors), where `x.y.z` is the same version of your
...
You can check the [Connector Modules](/v1.11.x/connectors) guide above to learn how to install the `openmetadata-ingestion` package with the
