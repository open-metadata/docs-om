...
- **DELETE** the admin default account shipped by OM in case you had [Basic Authentication](/v1.11.x/deployment/security/basic-auth)
  enabled before configuring the authentication with Keycloak SSO.
- **UPDATE** the Private / Public keys used for the [JWT Tokens](/v1.11.x/deployment/security/enable-jwt-tokens). The keys we provide
...
- [Implicit Flow](/v1.11.x/deployment/security/keycloak/implicit-flow) (Public)
- [Auth Code Flow](/v1.11.x/deployment/security/keycloak/auth-code-flow) (Confidential)
...
- **Web:**
  ...
  The [OIDC Authorization Code Flow](/v1.11.x/deployment/security/oidc) is used in this case, where the client secret is required to securely exchange the authorization code for tokens.
