...
* Secret Key: Pass the JSON file generated in [Create Service Account](/v1.12.x-SNAPSHOT/deployment/security/google) as the secretKey.
...
* Private Key: Use the Public/Private Key Pair that was generated while [Creating the Service Application](/v1.12.x-SNAPSHOT/deployment/security/okta#okta-sso). When copy-pasting the keys ensure that there are no additional codes and that it is a JSON compatible string.
