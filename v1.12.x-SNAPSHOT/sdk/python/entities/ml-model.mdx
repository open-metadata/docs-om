...
While we can already extract certain pieces of information automatically via our Connectors (e.g., [Mlflow](/v1.12.x-SNAPSHOT/connectors/ml-model/mlflow)), there are attributes that we'll need to fill in by ourselves. Thanks to the [Solution Design](/v1.12.x-SNAPSHOT/main-concepts/high-level-design) of OpenMetadata and the [Python SDK](/v1.12.x-SNAPSHOT/sdk/python), this is going to be a rather easy task that will unlock the full power of your **organization's metadata**.
...
```python
from metadata.sdk import configure, MLModels, Tables
from metadata.sdk.api import Lineage

configure(host="http://localhost:8585/api", jwt_token="<token>")

# Retrieve the ML model entity
my_mlmodel_entity = MLModels.retrieve_by_name("demo-ml-service.my-model")

# For each dataSource table referenced in the ML model features,
# add lineage from the source table to the ML model
source_table = Tables.retrieve_by_name("my-service.my-db.my-schema.my-source-table")

Lineage.add_lineage(
    from_entity_id=str(source_table.id),
    from_entity_type="table",
    to_entity_id=str(my_mlmodel_entity.id),
    to_entity_type="mlmodel",
    description="Feature source for ML model"
)
```

This registers the **Lineage** information between the source tables referenced in the dataSources and the MlModel instance. The relationships are then visible via the API or the UI.
...
In the [Solution Design](/v1.12.x-SNAPSHOT/main-concepts/high-level-design#example-1-updating-columns-of-a-table), we discussed the versioning example on how a Table reacts to adding or deleting columns in terms of its version. How do we manage versions for our MlModels?
