...
Snowflake natively supports Apache Iceberg tables. When you use the Snowflake connector in OpenMetadata, Iceberg tables stored in Snowflake are automatically ingested with full support for metadata extraction, data profiling, and data quality workflows — no separate Iceberg connector required.
</Info>
