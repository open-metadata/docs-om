...
For a user to log in using SSO, you must configure SSO from your identity provider to OpenMetadata.
...
- **Group Provisioning Behavior in OpenMetadata**:
    - **Group Adoption**: OpenMetadata searches for existing groups by name. If a group with the same name already exists in OpenMetadata (regardless of its location in the hierarchy), OpenMetadata will "adopt" that group and apply SCIM updates to it.
    - **New Group**: If your identity provider sends a group that doesn't yet exist in OpenMetadata, we'll create it at the root (under the Organization) with team type as "group"
    - **Existing Group**: If the group already exists—no matter where it sits in OpenMetadata's hierarchy—we'll simply add or remove the user in that group
- **Critical Distinction - Push Groups vs. App Assignment** (Okta):
    - **Push Groups**: Syncs the group structure itself (creates/updates the group entity in OpenMetadata)
...
> - The OpenMetadata team can help with TLS configuration if needed
...
3. Set the **SCIM API endpoint URL** to `https://yourcompany.open-metadata.org/api/v1/scim`
4. Set **Secret Token** to the OpenMetadata SCIM token that you generated in Step 1
...
- **Authentication method**: SCIM uses OAuth Bearer tokens (the SCIM token from OpenMetadata), not your SSO credentials
...
5. Give the application a meaningful name (e.g., "OpenMetadata SCIM Provisioning")
6. Click **Done**
...
4. Set the **Base URL** to `https://yourcompany.open-metadata.org/api/v1/scim`
5. Set the **API Token** (OAuth Bearer Token) to the OpenMetadata SCIM token from Step 1
...
> - Your OpenMetadata server has TLSv1.2 enabled (connection failures are often due to unsupported TLS versions)
> - Your network allows outbound connections from Okta to your OpenMetadata instance
...
2. Click **Edit** and enable the following:
    - **Create Users**: Enable to create users in OpenMetadata
...
> **Critical**: This step is what actually provisions users into OpenMetadata. Push Groups (Step 2B.5) only creates the group structure; this step provisions the users themselves.

1. Go to the **Assignments** tab
2. Click **Assign** → **Assign to Groups**
3. Select the groups containing users you want to provision to OpenMetadata
4. Click **Assign** for each group, then **Done**

**What this does**: Assigning a group to the SCIM app authorizes Okta to provision the users within that group to OpenMetadata. Without this assignment, users will NOT be provisioned, even if you push the group structure.
...
> - **Push Groups**: Syncs the group entity/structure to OpenMetadata (creates/updates the group itself)
...
3. Configure group provisioning options:
    ...
    - **Create Groups**: Enable to automatically create groups in OpenMetadata
    - **Update Group Attributes**: Enable to sync group changes
4. For automatic provisioning, you can set up **Push groups by name/rule**:
    - Define rules like groups starting with "OM_" or "OpenMetadata_"
...
> 1. **Push the group** (this step) - to create the group structure in OpenMetadata
...
1. **Connection Failures**:
    - **TLS Version Issues**: Most connection failures are caused by unsupported TLS versions. Ensure TLSv1.2 is enabled on your OpenMetadata server
    - Verify the SCIM token is correct and hasn't expired
    - Check that your OpenMetadata instance URL is accessible from Okta's network
...
4. **Group Adoption Questions**:
    - If you manually created a group in OpenMetadata and later provision it via SCIM, OpenMetadata will search for the group by name
    - If a match is found, OpenMetadata "adopts" the existing group and subsequent SCIM updates (like adding members) apply to that group
