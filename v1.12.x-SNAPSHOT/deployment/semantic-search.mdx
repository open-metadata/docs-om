...
entity type, tags, glossary terms, owners, and other relevant fields. This text is then sent to the configured
embedding provider to generate a numerical vector (embedding), which is stored in a dedicated OpenSearch
`vector_search_index` using the HNSW algorithm with cosine similarity.

At query time, the search text is also embedded and a KNN (K-Nearest Neighbor) similarity search finds the most
relevant results.
