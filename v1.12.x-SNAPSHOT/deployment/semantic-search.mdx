...
queries, rather than relying solely on keyword matching. When enabled, users and AI agents can search using natural language
-- for example, *"tables with customer demographics and purchase history"* -- and get meaningful results even if those exact
words don't appear in the metadata.
...
Semantic Search operates through a simple pipeline:

1. **Text Construction**: For each entity, we construct text from its metadata (name, description, entity type, service type, fully qualified name, tags, glossary terms, owners, tier, domains, and entity-specific fields).

2. **Embedding Generation**: The text is sent to the configured embedding provider (OpenAI, AWS Bedrock, or DJL) which returns a numerical vector representation.

3. **Vector Indexing & Storage**: Each vector is stored in a dedicated OpenSearch vector index with metadata for filtering, using the HNSW (Hierarchical Navigable Small World) algorithm for fast similarity search.

4. **Automatic Lifecycle Management**: Embeddings are automatically created, updated, deleted, or restored in sync with entity lifecycle events using the same indexing strategies OpenMetadata already uses. No manual intervention is required after initial setup.
...
<Tabs>
  ...
  <Tab title="DJL (Testing/Development Only)">
    Uses [Deep Java Library](https://djl.ai/) to run embedding models locally. No external API calls required.

    <Warning>
    DJL is intended for **testing and development only**. For production environments, we recommend **OpenAI** or **AWS Bedrock**.
    </Warning>
...
Embeddings are automatically generated and maintained for all entities. When entities are created, updated, or deleted,
the platform automatically computes embeddings using the configured embedding provider.

To optimize performance and avoid unnecessary API calls, each reindex operation computes a fingerprint of the entity text.
If the fingerprint matches the existing one, embedding generation is skipped. This ensures embeddings are only recomputed
when entity content actually changes.
