...
<CardGroup cols={2}>
  ...
  <Card title="Kubernetes Native" href="/v1.12.x-SNAPSHOT/deployment/ingestion/kubernetes" icon="cube">
...
We have specific instructions on how to set up the shared volumes in Kubernetes depending on your cloud deployment [here](/v1.12.x-SNAPSHOT/deployment/kubernetes).
...
And then run the DAG as explained in each [Connector](/v1.12.x-SNAPSHOT/connectors), where `x.y.z` is the same version of your
...
You can check the [Connector Modules](/v1.12.x-SNAPSHOT/connectors) guide above to learn how to install the `openmetadata-ingestion` package with the
...
- If you have an existing Airflow instance, and you want to build and maintain your own ingestion DAGs then you can go for it. Check a DAG example [here](/v1.12.x-SNAPSHOT/deployment/ingestion/external/airflow#example).
...
<CardGroup cols={1}>
<Card title="Airflow SSL" href="/v1.12.x-SNAPSHOT/deployment/security/enable-ssl/airflow-ssl" >
