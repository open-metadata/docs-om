...
description: 'Complete API reference for the OpenMetadata Python SDK'
---

# Python SDK API Reference

The OpenMetadata Python SDK provides a high-level interface for interacting with the OpenMetadata API. It is based on generated Pydantic models from JSON schemas and provides a typed approach to working with OpenMetadata entities.

## Core Classes

### OpenMetadata

The main client class that provides a generic interface to the OpenMetadata API. It implements polymorphism across all different entities and includes specific functionalities inherited from mixins.
...
# Initialize client
metadata = OpenMetadata(config: OpenMetadataConnection, raw_data: bool = False)
```

#### Constructor

- **`config`** (`OpenMetadataConnection`): Connection configuration
...
1. **Reuse Client Instances**: Create one `OpenMetadata` client and reuse it
