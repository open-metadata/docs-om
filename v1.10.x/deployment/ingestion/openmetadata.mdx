...
We have specific instructions on how to set up the shared volumes in Kubernetes depending on your cloud deployment [here](/v1.10.x/deployment/kubernetes).
...
And then run the DAG as explained in each [Connector](/v1.10.x/connectors), where `x.y.z` is the same version of your
...
You can check the [Connector Modules](/v1.10.x/connectors) guide above to learn how to install the `openmetadata-ingestion` package with the
...
- If you have an existing Airflow instance, and you want to build and maintain your own ingestion DAGs then you can go for it. Check a DAG example [here](/v1.10.x/deployment/ingestion/external/airflow#example).
...
<CardGroup cols={1}>
<Card title="Airflow SSL" href="/v1.10.x/deployment/security/enable-ssl/airflow-ssl" >
