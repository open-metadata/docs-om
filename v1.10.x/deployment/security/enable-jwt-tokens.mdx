...
When we [enable SSO security](/v1.10.x/deployment/security) on OpenMetadata, it will restrict access to all the APIs. Users who want to access the UI
...
supports service accounts that the SSO provider supports. Please read the [docs](/v1.10.x/deployment/security) to enable them.
...
- **DELETE** the admin default account shipped by OM in case you have [Basic Authentication](/v1.10.x/deployment/security/basic-auth) enabled.
