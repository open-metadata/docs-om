...
Make sure to create database and search engine credentials as Kubernetes Secrets mentioned [here](/v1.10.x/quick-start/local-kubernetes-deployment#2.-create-kubernetes-secrets-required-for-helm-charts).
...
Starting with **OpenMetadata v1.10.4**, the dependency Helm chart no longer supports passing database passwords using individual Kubernetes secret keys (for example, `passwordSecret` and `passwordSecretKey`).
