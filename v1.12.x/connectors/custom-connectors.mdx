...
Review the [Sample Data source](https://github.com/open-metadata/OpenMetadata/blob/1.12.14-release/ingestion/src/metadata/ingestion/source/database/sample_data.py) to see a complete, release-aligned custom source implementation.
...
all the required methods in the [Source API reference](/v1.12.x/api-reference/sdk/python/build-connector/source#for-consumers-of-openmetadata-ingestion-to-define-custom-connectors-in-their-own-package-with-same-namespace).

Review the [Sample Data source](https://github.com/open-metadata/OpenMetadata/blob/1.12.14-release/ingestion/src/metadata/ingestion/source/database/sample_data.py) for a complete, release-aligned `Source` implementation.

The important method is `_iter`. This generator function sends Create Entity Requests to the `Sink`. Read more about the `Workflow` in the [Python SDK reference](/v1.12.x/api-reference/sdk/python/build-connector).
...
the different Entities, a recommended read is the Python SDK [docs](/v1.12.x/sdk/python).

We do not have docs and examples of all the supported Services. To see how to create and fetch
other types of Entities, refer to the `ometa` [integration tests](https://github.com/open-metadata/OpenMetadata/tree/1.12.14-release/ingestion/tests/integration/ometa).
