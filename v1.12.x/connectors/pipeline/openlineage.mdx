...
Each dataset in an OpenLineage event is resolved as follows:

1. **First symlink identifier** — if the event includes a `symlinks` facet with at least one identifier, the connector uses the **name from the first symlink** to look up the table. All symlink types (including `LOCATION`) are considered; there is no type-based filtering in this version.
2. **Top-level namespace and name** — used when no symlinks are present in the event.

<Info>
If the first symlink identifier cannot be matched to an existing OpenMetadata entity, no further candidates are tried and the dataset is skipped. Ensure the symlink name in the event matches an ingested table.
</Info>

### Hive / Glue Tables on Object Storage

When Spark writes to a table in a Glue or Hive catalog backed by S3, the OpenLineage event carries both an S3 path as the top-level namespace and a `symlinks` facet with catalog table identifiers. The connector uses the name from the **first symlink identifier** in the event to resolve the table.

<Tip>
Ensure your Glue or Hive catalog tables are ingested into OpenMetadata before running the OpenLineage pipeline. The first symlink name in the event must match the ingested table name exactly.
</Tip>
