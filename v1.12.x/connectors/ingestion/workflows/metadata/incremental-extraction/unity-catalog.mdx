...
The following queries run at the end of each ingestion to identify tables that have changed or been deleted since the last successful run. OpenMetadata replaces `{start_timestamp}` with the timestamp of the previous run as a millisecond integer epoch (for example, `1718000000000`).
