...
All available OpenMetadata MCP tools, with parameters and examples.

## Available Tools

| # | Tool | Category | Description |
|---|------|----------|-------------|
| 1 | `search_metadata` | Discovery | Keyword-based search for data assets and business terms |
| 2 | `semantic_search` | Discovery | Meaning-based search using vector embeddings |
| 3 | `get_entity_details` | Discovery | Retrieve full details for a specific entity by FQN |
| 4 | `get_entity_lineage` | Lineage | Explore upstream/downstream dependencies |
| 5 | `create_lineage` | Lineage | Create a lineage relationship between two entities |
| 6 | `get_test_definitions` | Data Quality | List available test definitions for tables or columns |
| 7 | `create_test_case` | Data Quality | Create a data quality test for a table or column |
| 8 | `create_glossary` | Governance | Create a new glossary to organize business terms |
| 9 | `create_glossary_term` | Governance | Add a term to an existing glossary |
| 10 | `create_metric` | Governance | Create a new metric entity |
| 11 | `patch_entity` | Governance | Update an existing entity using JSON Patch |
| 12 | `root_cause_analysis` | Data Quality | Perform root cause analysis via data quality lineage |
...
**Description**: Find data assets by meaning using vector search. Use for exploratory queries where you don't know exact names. Returns conceptually related assets even when no keywords match.
...
| `filters` | object | No | Optional filters: `entity_type`, `service`, `tags` (each an array of strings) |
...
```json
{
  ...
  "params": {
    ...
    "arguments": {
      ...
      "fqn": "mysql_prod.ecommerce.public.customer_orders",
      "patch": [
        { "op": "replace", "path": "/description", "value": "Updated description for customer orders table." }
      ]
    }
  }
}
