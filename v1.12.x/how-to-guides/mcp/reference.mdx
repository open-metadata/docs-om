...
**Description**: Find data assets by meaning using vector search (For more information, see our setup guide [here](/deployment/semantic-search#enable-semantic-search-openmetadata-deployment-guide)). Use for exploratory queries where you don't know exact names. Returns conceptually related assets even when no keywords match.
