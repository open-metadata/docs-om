...
There is no separate lineage agent or lineage pipeline—lineage is collected as part of the same metadata ingestion workflow. However, you do need to configure the **Db Service Prefixes** field (covered below) so OpenMetadata knows which database services to match against your dashboards.
...
5. In the **Db Service Prefixes** field, enter the name of the database service in OpenMetadata that your dashboards connect to.

This tells OpenMetadata where to look for the source tables so it can draw the lineage path from table to dashboard.
