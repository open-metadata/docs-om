...
- **Include Views (toggle)**: Ingest metadata for views in addition to tables. Turn off to skip views entirely. Since view lineage is derived from parsing each view's SQL definition, turning this off also stops view-based lineage from being generated for this service. On by default.
...
- **Hierarchical Owner Configuration**: A dedicated section for assigning owners to databases, schemas, and tables during ingestion, with per-entity mappings and inheritance. For more information about how resolution works, see [Hierarchical Owner Configuration](/v1.13.x/connectors/ingestion/workflows/metadata/hierarchical-owner-configuration).
    ...
    - **Enable Inheritance (toggle)**: When enabled (default), a schema or table without its own match inherits the resolved owner from its parent.
...
- **Extract JSON Schema (toggle)**: Sample values in JSON columns to infer and ingest their schema, so the individual fields inside a JSON column are visible in OpenMetadata rather than just "JSON". Requires `SELECT` permission on the sampled tables. If that permission is missing, the JSON column is still ingested, just without a detailed schema. Off by default.
- **JSON Schema Sample Size (Optional)**: Only used when Extract JSON Schema is on. Number of rows sampled per JSON column to infer its schema. Defaults to 10.
