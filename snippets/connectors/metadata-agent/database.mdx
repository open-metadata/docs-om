...
4. On the **Configure Ingestion** page, do the following and click **Next**.
   ...
   - **Agent Setup**: Configure core parameters for metadata extraction. The following fields are available:
        ...
        | Safety Margin Days | 1 | Only used when Incremental Extraction is on. Additional days added to the baseline timestamp as a buffer for incremental extraction. |
        ...
        | JSON Schema Sample Size | 10 | Only used when Extract JSON Schema is on. Number of rows sampled per JSON column to infer its schema. |
