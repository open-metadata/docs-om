...
- A card for each article, showing its title, a short description (or "No description" if none is set), its last-edited time, its owner, its domain, and any tags applied to it.
...
- **Toolbar actions**, next to the Saved indicator:
  - **Feedback**: Give a thumbs up or thumbs down on the article.
...
- **Content** and **Activity Feeds & Tasks** tabs:
  ...
  - **Activity Feeds & Tasks** tracks everything that's happened on the article, split into two views:
    - **All**: A chronological feed of every change, showing who created or updated the article, when, and a short description of what changed. React to an entry with an emoji, or reply to start a thread on it.
