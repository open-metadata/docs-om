...
The Semantic Search tool extends OpenMetadata's [MCP Server](/v2.0.x/how-to-guides/mcp) with the ability to
...
configuration](/v2.0.x/deployment/semantic-search) before it can be used through MCP.
...
- Semantic Search [enabled and configured](/v2.0.x/deployment/semantic-search) in your deployment
- The [MCP Application installed](/v2.0.x/how-to-guides/mcp#installing-mcp-server) in OpenMetadata
- A [Personal Access Token](/v2.0.x/how-to-guides/mcp#personal-access-token-pat)
  for authentication
...
| Configuration | Available by default | Requires [enabling](/v2.0.x/deployment/semantic-search) and an embedding provider |
...
- Ensure entities have been embedded by running a Reindex from the OpenMetadata UI. See the
  [deployment guide](/v2.0.x/deployment/semantic-search#generating-embeddings) for details.
