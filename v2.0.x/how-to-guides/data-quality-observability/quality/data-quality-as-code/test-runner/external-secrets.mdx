...
Secrets manager client libraries are included in the base ingestion package. Install any connector-specific extras required by your test YAML separately, for example `openmetadata-ingestion[mysql,snowflake]~=2.0.1.0`.
...
```bash
pip install "openmetadata-ingestion~=2.0.1.0"
pip install "openmetadata-ingestion[mysql,snowflake,s3]~=2.0.1.0"
```
