...
<Tabs>
  ...
  <Tab title="AWS Bedrock">
    ...
    | Environment Variable | Default | Description |
    |----------------------------------------------|---------|-------------|
    | `BEDROCK_AWS_IAM_AUTH_ENABLED` | `true` | Use the server's IAM role instead of static credentials |
    | `AWS_DEFAULT_REGION` | `""` | AWS region |
    | `AWS_ACCESS_KEY_ID` | `""` | AWS access key (only needed when IAM auth is disabled) |
    | `AWS_SECRET_ACCESS_KEY` | `""` | AWS secret access key (only needed when IAM auth is disabled) |
    | `AWS_SESSION_TOKEN` | `""` | AWS session token (only needed when IAM auth is disabled) |
...
For a faster, dedicated alternative to a full Reindex, use the `reembed` CLI command to drop and recreate the vector
index and re-embed every entity:
