...
sidebarTitle: Configuration Reference Parameters
collate: false
---
...
Also if you have enabled [JWT Tokens](/v2.0.x/deployment/security/enable-jwt-tokens) then `http://localhost:8585/api/v1/system/config/jwks` also needs to be there in the list with proper server url.
...
Default: `"3600"` (1 hour)
Must be a positive value. OpenMetadata rejects a zero or negative value, and falls back to `3600` at runtime if an invalid value is ever persisted.
...
Whether to derive roles from the authentication provider. Roles must be included in the identity provider's token and must match existing OpenMetadata role names. OpenMetadata skips any role name it doesn't recognize.

Whether a role change at the provider reaches OpenMetadata depends on how the user signs in and which 2.0.x release you run:

- **OIDC sign-in on OpenMetadata 2.0.2**: OpenMetadata updates the user's roles each time they sign in, so a role added or removed at the provider takes effect at their next sign-in. If the token doesn't include a roles claim at all, OpenMetadata leaves the user's existing roles unchanged.
- **OIDC sign-in on OpenMetadata 2.0.0 and 2.0.1**: Signing in doesn't update roles from the provider. The user keeps the roles already stored in OpenMetadata, so change them in OpenMetadata directly.
- **SAML sign-in**: Signing in doesn't update roles from the provider in any 2.0.x release. Manage these users' roles in OpenMetadata directly.

The provider can make a user an admin, but removing the Admin role at the provider doesn't remove their admin access. To remove it, update the user in OpenMetadata. If the user is also listed in `adminPrincipals`, remove them from that list too, because OpenMetadata restores admin access to everyone on it when they sign in.
...
<Note>
**Note:** At least one claim in this list must correspond to the user's **email
address** — OpenMetadata uses this to identify and match authenticated users to
their accounts. Use `email` for consistency and compatibility; for domain
scoping, use the `hd` claim.
</Note>
