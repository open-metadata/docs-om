...
ODCS documents can carry data OpenMetadata doesn't model directly, such as quality rules or element-level
`authoritativeDefinitions` and `transformSourceObjects`. Rather than dropping this data, OpenMetadata stores it
as a passthrough and restores it on export.
...
- Several other ODCS attributes aren't mapped yet and don't round-trip: `businessName`, `classification`,
  ...
  `maxLength`, which round-trips through the column's `dataLength`), and element-level `tags`.
