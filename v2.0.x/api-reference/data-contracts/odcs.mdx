...
as a passthrough and restores it on export, so a round trip through OpenMetadata doesn't lose it.
...
- Several other ODCS attributes aren't mapped yet and don't round-trip: `businessName`, `classification`,
  `examples`, `customProperties`, `transformLogic`, `transformDescription`, `logicalTypeOptions`, and
  element-level `tags`.
