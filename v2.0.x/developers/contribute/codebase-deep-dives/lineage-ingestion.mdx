...
Lineage uses the same query log sources as [usage ingestion](/v2.0.x/developers/contribute/codebase-deep-dives/usage-ingestion), but processes queries differently — it analyzes **directional data flow** (source → target) rather than counting references.
...
<Info>
For entity resolution, cross-database lineage, stored procedures, parallel processing, publishing, dialect mapping, and configuration, see [Lineage Ingestion — Advanced Topics](/v2.0.x/developers/contribute/codebase-deep-dives/lineage-ingestion/advanced).
</Info>
