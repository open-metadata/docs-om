...
# Add a follower
topic = client.put(f"/topics/{topic_id}/followers", json=user_id)
print(f"Followers: {len(topic['followers'])}")

# Remove a follower
topic = client.delete(f"/topics/{topic_id}/followers/{user_id}")

# Add a vote
topic = client.put(f"/topics/{topic_id}/vote", json={"updatedVoteType": "votedUp"})

# Remove a vote
topic = client.put(f"/topics/{topic_id}/vote", json={"updatedVoteType": "unVoted"})
```
