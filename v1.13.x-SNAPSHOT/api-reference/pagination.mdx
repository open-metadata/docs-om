...
|-----------|------|---------|-------------|
| `limit` | integer | 10 | Number of results per page (max 1,000,000) |
| `before` | string | - | Cursor for previous page |
| `after` | string | - | Cursor for next page |
| `offset` | integer | 0 | Number of results to skip. Available on sub-resource endpoints (for example, `/v1/tables/{id}/columns`). Not supported on main list endpoints – use `before`/`after` cursors instead. |
...
| `offset` | integer | Current offset position (returned on sub-resource endpoints only) |
...
Use `offset` on sub-resource endpoints such as `/v1/tables/{id}/columns`:
...
<CardGroup cols={2}>
  <Card title="Search API" icon="magnifying-glass" href="/v1.13.x-SNAPSHOT/api-reference/data-assets/search-indexes" horizontal>
