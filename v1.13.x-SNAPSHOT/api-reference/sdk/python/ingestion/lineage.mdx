...
All functions that we are going to use related to Lineage can be found in the [SDK Lineage API](https://github.com/open-metadata/OpenMetadata/blob/main/ingestion/src/metadata/sdk/api/lineage.py).
...
lineage = Lineage.get_entity_lineage(
    entity_type="table",
    entity_id=str(table_b_entity.id),
...
See the [Lineage Ingestion](/v1.13.x-SNAPSHOT/connectors/pipeline) documentation for details.
</Info>
