...
updated_table = Tables.add_tag(table.id, "TestClassification.TestTag")
```
...
assign tags in the `tags` property of `CreateTableRequest` directly — no PATCH needed.

### 9. Tagging a Column

For column-level tags, use the REST API PATCH endpoint. The entity facade does not currently expose a column tag helper.
