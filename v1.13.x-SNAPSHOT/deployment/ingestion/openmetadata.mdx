...
<CardGroup cols={2}>
  ...
  <Card title="Kubernetes Native" href="/v1.13.x-SNAPSHOT/deployment/ingestion/kubernetes" icon="cube">
...
And then run the DAG as explained in each [Connector](/v1.13.x-SNAPSHOT/connectors), where `x.y.z` is the same version of your
...
You can check the [Connector Modules](/v1.13.x-SNAPSHOT/connectors) guide above to learn how to install the `openmetadata-ingestion` package with the
