...
For example, if you are using the [Glue](/v1.13.x-SNAPSHOT/connectors/database/glue) connector you could specify the
...
You can validate the whole Google service account setup [here](/v1.13.x-SNAPSHOT/deployment/security/google).
...
with DAG(
    ...
    description="An example DAG which runs an OpenMetadata ingestion workflow",
