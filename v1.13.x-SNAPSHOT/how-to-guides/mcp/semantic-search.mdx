...
The Semantic Search tool extends OpenMetadata's [MCP Server](/v1.13.x-SNAPSHOT/how-to-guides/mcp) with the ability to
...
configuration](/v1.13.x-SNAPSHOT/deployment/semantic-search) before it can be used through MCP.
...
- Semantic Search [enabled and configured](/v1.13.x-SNAPSHOT/deployment/semantic-search) in your deployment
- The [MCP Application installed](/v1.13.x-SNAPSHOT/how-to-guides/mcp#installing-mcp-server) in OpenMetadata
- A [Personal Access Token](/v1.13.x-SNAPSHOT/how-to-guides/mcp#adding-a-personal-access-token-to-your-mcp-client)
  for authentication
...
| Configuration | Available by default | Requires [enabling](/v1.13.x-SNAPSHOT/deployment/semantic-search) and an embedding provider |
...
- Ensure entities have been embedded by running a Reindex from the OpenMetadata UI. See the
  [deployment guide](/v1.13.x-SNAPSHOT/deployment/semantic-search#generating-embeddings) for details.
