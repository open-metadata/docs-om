...
The `LineageParser` (`lineage/parser.py`) is shared with [lineage ingestion](/v1.13.x-SNAPSHOT/developers/contribute/codebase-deep-dives/lineage-ingestion) — usage ingestion uses the **table extraction** capabilities, while lineage ingestion additionally uses the **source/target direction** analysis.
