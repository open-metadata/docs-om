...
```sql
-- Enable the extension (requires superuser). Also add to postgresql.conf:
...
If your organization restricts direct access to `pg_stat_statements`, or if you need more control over query retention, you can set the **Query Statement Source** connection property to a custom view or table (e.g., `my_schema.custom_query_history`). The custom source must expose the columns: `userid` (oid), `dbid` (oid), `query` (text), and `total_exec_time` (double precision). If not set, OpenMetadata defaults to `pg_stat_statements`.
### IAM Authentication
