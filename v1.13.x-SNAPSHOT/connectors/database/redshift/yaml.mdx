...
<ContentSection id={3} title="Auth Type" lines="8-12">

**authType**: Choose the authentication method:

- **Basic Auth** (`password`): Use username and password for authentication.
- **IAM Auth** (`awsConfig`): Use AWS IAM for authentication. The connector will retrieve temporary credentials using `GetClusterCredentials` (Provisioned) or `GetCredentials` (Serverless) APIs.
...
source:
  ...
  serviceConnection:
    config:
      ...
      authType:
        password: password
      # Option 2: IAM Auth (uncomment below and comment out password above)
      # authType:
      #   awsConfig:
      #     awsRegion: us-east-1  # REQUIRED for IAM auth
      ...
      #     # assumeRoleArn: arn:aws:iam::123456789012:role/role-name  # For cross-account
