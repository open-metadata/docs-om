...
<ContentSection id={5} title="Oracle Connection Type" lines="11-14">
...
- **Use DBA Table (toggle)**: When enabled, connector uses `DBA_TABLES` to retrieve metadata for all database objects. This requires the Oracle user to have DBA privileges. When disabled, connector uses  `ALL_TABLES`, which only returns metadata for objects accessible to the current user.
...
<ContentSection id={6} title="Database Name" lines="15">
...
<ContentSection id={7} title="Connection Options" lines="16-17">
...
<ContentSection id={8} title="Connection Arguments" lines="18-19">
