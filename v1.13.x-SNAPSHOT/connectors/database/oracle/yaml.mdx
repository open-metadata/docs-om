...
<ContentSection id={9} title="Use DBA Table" lines="19">

**useDBATable**: When enabled, connector uses `DBA_TABLES` to retrieve metadata for all database objects. This requires the Oracle user to have DBA privileges. When disabled, connector uses `ALL_TABLES`, which only returns metadata for objects accessible to the current user.
