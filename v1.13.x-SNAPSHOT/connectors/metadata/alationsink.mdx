...
- [Troubleshooting](/v1.13.x-SNAPSHOT/connectors/metadata/alationsink/troubleshooting)
<ExternalIngestionDeployment />
## Requirements
The connector uses `POST` requests to write the data into Alation.
Hence, a user's credentials or an access token with `Source Admin` or `Catalog Admin` or `Server Admin` permissions will be required.
