...
Regardless of the Airflow ingestion process you follow ([Workflow](/v1.13.x-SNAPSHOT/connectors/pipeline/airflow),
[Lineage Backend](/v1.13.x-SNAPSHOT/connectors/pipeline/airflow/lineage-backend) or [Lineage Operator](/v1.13.x-SNAPSHOT/connectors/pipeline/airflow/lineage-operator)),
OpenMetadata will try to extract the lineage information based on the tasks `inlets` and `outlets`.

What is important to consider here is that when we are ingesting Airflow lineage, we are actually building a graph:
