...
The OpenLineage connector is **read-only** — it resolves lineage edges to entities already ingested in OpenMetadata. Tables, schemas, and databases are never created automatically. **Ingest your database sources before running the OpenLineage pipeline** or lineage edges will not attach.
