...
1. **[REST API connection](/v2.1.x-SNAPSHOT/connectors/pipeline/airflow/rest-api-connection#4-mwaa-configuration-amazon-managed-workflows-for-apache-airflow) (recommended).** The ingestion runs from OpenMetadata (UI-scheduled or via the CLI) and reads DAGs through the MWAA web server, using AWS credentials. No access to the underlying database is needed, and nothing has to be installed inside MWAA.
2. **Running the ingestion inside MWAA.** A DAG deployed in MWAA extracts metadata from the local Airflow backend and pushes it to OpenMetadata. Use this when your MWAA environment runs an Airflow version older than 2.4.3, or when the ingestion cannot be given AWS credentials.
...
OpenMetadata authenticates by exchanging your AWS credentials for a short-lived Airflow web login token (`airflow:CreateWebLoginToken`), then reads DAGs, runs, and task states through the MWAA REST API. OpenMetadata detects the Airflow API version automatically (v1 on Airflow 2, v2 on Airflow 3).

### Requirements

- The MWAA environment must run **Airflow 2.4.3 or newer** (the minimum version supported by the MWAA `InvokeRestApi` API).
- The IAM identity used in the connection needs `airflow:CreateWebLoginToken` and `airflow:InvokeRestApi` on the environment:

```json
{
  "Effect": "Allow",
  "Action": ["airflow:CreateWebLoginToken", "airflow:InvokeRestApi"],
  "Resource": "arn:aws:airflow:<region>:<account-id>:environment/<environment-name>"
}
```
...
For the full parameter reference, see the [REST API Connection documentation](/v2.1.x-SNAPSHOT/connectors/pipeline/airflow/rest-api-connection).

<Info>
The REST API alone doesn't capture table-level lineage. It requires the OpenLineage provider configured in MWAA. For the required setup, see the [OpenLineage Setup Summary](/v2.1.x-SNAPSHOT/connectors/pipeline/airflow/rest-api-connection#openlineage-setup-summary).
</Info>

## 2. Running the Ingestion Inside MWAA

If the REST API connection isn't an option, run the ingestion as a DAG inside MWAA using the `Backend` connection, which reads MWAA's own metadata database directly. The OpenMetadata server must be reachable **from** MWAA. For more information about running connectors from MWAA, see [Run the ingestion from AWS MWAA](/v2.1.x-SNAPSHOT/deployment/ingestion/external/mwaa). There are three supported approaches:

1. Install the `openmetadata-ingestion` package as a requirement in the Airflow environment, then run the process using a `PythonOperator`.
