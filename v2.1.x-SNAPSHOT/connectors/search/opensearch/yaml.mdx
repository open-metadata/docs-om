...
<CodePreview>
  <ContentPanel>
    <ContentSection id={1} title="hostPort" lines="7">
      <strong>hostPort</strong>: This parameter specifies the host and port of the OpenSearch instance. This should be specified as a URI string in the format `http://<hostname>:<port>` or `https://<hostname>:<port>`. For example, you might set it to `https://localhost:9200`.
    </ContentSection>
