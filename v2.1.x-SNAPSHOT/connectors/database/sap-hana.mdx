...
SAP HANA lineage is built from three sources. Each one needs a different grant, and each covers a different kind of relationship, so the grants you need depend on the lineage you expect to see.

| Lineage | Read from | Grant required |
|---|---|---|
| View lineage, including column level | `SYS.VIEWS` | `SELECT` on `SYS`, already required for metadata |
| Table to table, from `INSERT INTO ... SELECT` and similar | `SYS.M_SQL_PLAN_CACHE` | `CATALOG READ` |
| Calculation, Analytic and Attribute View lineage | `_SYS_REPO.ACTIVE_OBJECT` | `SELECT` on `_SYS_REPO` |

```SQL
-- Table-to-table lineage. Without this the ingestion user sees only the statements
-- it ran itself, so the lineage workflow finds nothing to parse.
GRANT CATALOG READ TO <user_or_role>;

-- Calculation, Analytic and Attribute View lineage. On-premise and HANA Express only.
GRANT SELECT ON SCHEMA _SYS_REPO TO <user_or_role>;
```

<Note>
**SAP HANA Cloud.** `_SYS_REPO` is part of the classic HANA repository, which was never carried into SAP HANA Cloud. On Cloud there are no Calculation, Analytic or Attribute Views to read, so that grant does not apply and the lineage workflow logs an informational message saying the schema is absent. View, column-level and table-to-table lineage are unaffected.
</Note>
