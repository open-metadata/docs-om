...
<Note>
**SAP HANA Cloud.** `_SYS_REPO` is the classic HANA repository, which was never carried into SAP HANA Cloud, so that grant does not apply there and the lineage workflow logs an informational message saying the schema is absent. This affects only classic repository models. Calculation views built in HDI containers are still ingested through `_SYS_BIC`, and view, column-level, and table-to-table lineage are unaffected.
</Note>

<Note>
**`CREATE TABLE ... AS SELECT` is not captured.** SAP HANA records lineage-relevant statements in the plan cache, which holds execution plans and therefore never contains DDL. A table populated by `CREATE TABLE ... AS SELECT` will have no upstream lineage. Populating it with `INSERT INTO ... SELECT` instead makes the relationship visible.
</Note>
