...
SQL Server synonyms are ingested as an alternate name on the table or view they point to, not as separate entities. OpenMetadata sweeps `sys.synonyms` across every in-scope database before ingesting tables, so a synonym that points to a table in a different database resolves correctly, as long as both databases are in scope.
