...
description: Learn to configure dbt workflows in OpenMetadata with our step-by-step guide. Streamline data lineage, documentation, and metadata management effortlessly.
...
See the [Storage Configuration Overview](/v2.1.x-SNAPSHOT/connectors/database/dbt/storage-configuration-overview) for setup guides:
- [AWS S3](/v2.1.x-SNAPSHOT/connectors/database/dbt/storage-s3-guide) | [Google Cloud Storage](/v2.1.x-SNAPSHOT/connectors/database/dbt/storage-gcs-guide) | [Azure Blob](/v2.1.x-SNAPSHOT/connectors/database/dbt/storage-azure-guide) | [HTTP Server](/v2.1.x-SNAPSHOT/connectors/database/dbt/storage-http-guide) | [Local/Shared Filesystem](/v2.1.x-SNAPSHOT/connectors/database/dbt/storage-local-guide)
...
We can create a workflow that will obtain the dbt information from the dbt files and feed it to OpenMetadata. The dbt Agent will be in charge of obtaining this data.

### Step 1: Add a dbt Agent

In the Service details page, navigate to the **Agents** tab, click **Add Agent**, and select **Add dbt Agent**.

<img src="/public/images/connector2.0/agents/add-dbt-agent.png" alt="Add dbt Agent" />

### Step 2: Configure the dbt Agent
...
Follow the link [here](/v2.1.x-SNAPSHOT/connectors/database/dbt/setup-multiple-dbt-projects) for instructions on setting up multiple dbt projects.

<img src="/public/images/connector2.0/dbt/dbt-aws-s3-config.png" alt="aws-s3-bucket" />
...
Follow the link [here](/v2.1.x-SNAPSHOT/connectors/database/dbt/setup-multiple-dbt-projects) for instructions on setting up multiple dbt projects.
    ...
    <img src="/public/images/connector2.0/dbt/dbt-gcs-config.png" alt="gcp-storage-bucket-form" />


- Entering the path of file in which the GCS bucket credentials are stored.

    <img src="/public/images/connector2.0/dbt/dbt-gcs-path.png" alt="gcp-storage-bucket-path" />
...
<img src="/public/images/connector2.0/dbt/dbi-azure-config.png" alt="azure-bucket" />
...
<img src="/public/images/connector2.0/dbt/dbt-local-config.png" alt="local-storage" />

#### File Server

File server path of the `manifest.json`, `catalog.json` and `run_results.json` files stored on a file server directly provided.

<img src="/public/images/connector2.0/dbt/dbt-http-config.png" alt="file-server" />
...
<img src="/public/images/connector2.0/dbt/dbt-cloud-config.png" alt="dbt-cloud" />

<Tip>

The fields for `Dbt Cloud Account Id`, `Dbt Cloud Project Id` and `Dbt Cloud Job Id` should be numeric values.

To know how to get the values for `Dbt Cloud Account Id`, `Dbt Cloud Project Id` and `Dbt Cloud Job Id` fields check [here](/v2.1.x-SNAPSHOT/connectors/database/dbt/run-dbt-workflow-externally).
...
<img src="/public/images/connector2.0/dbt/dbt-add-deploy.png" alt="schedule-and-deploy" />
