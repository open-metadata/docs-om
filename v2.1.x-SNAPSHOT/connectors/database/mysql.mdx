...
Views need both privileges because MySQL gates each half of a view read separately. `SHOW VIEW` authorizes `SHOW CREATE TABLE`, which returns the view definition. `SELECT` authorizes `DESCRIBE`, which returns the view columns. Holding only one still triggers `Error 1142` for the missing half, but OpenMetadata catches that per view rather than failing the workflow. The view entity is still created, just missing its schema definition (DDL) if `SHOW VIEW` is missing, or its columns if `SELECT` is missing.
...
Object-level grants also limit discovery. `SHOW TABLES` returns only the objects the user holds some privilege on, so any table or view left out of the grant never reaches OpenMetadata and raises no error. Stored procedures behave the same way. They stay hidden in `INFORMATION_SCHEMA.ROUTINES` until the user holds `SHOW_ROUTINE`.
