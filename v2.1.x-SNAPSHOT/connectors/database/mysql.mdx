...
| Stored procedures | No | No | No, `EXECUTE` is also required |

Views need both privileges because MySQL gates each half of a view read separately. `SHOW VIEW` authorizes `SHOW CREATE TABLE`, which returns the view definition, and `SELECT` authorizes `DESCRIBE`, which returns the view columns. Holding only one raises `Error 1142` on every view. OpenMetadata creates no entity for that view, and the workflow finishes with errors.
