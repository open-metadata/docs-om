...
Grant both `SELECT` and `SHOW VIEW`. Either privilege on its own already ingests base tables in full, which is what makes an incomplete grant easy to miss:

| Capability | `SELECT` only | `SHOW VIEW` only | Both |
| --- | --- | --- | --- |
| Table discovery, columns, comments, keys, indexes, and DDL | Yes | Yes | Yes |
| Views | No | No | Yes |
| Profiler, sample data, and data quality | Yes | No | Yes |
| Stored procedures | No | No | No, `SHOW_ROUTINE` is also required |
