...
For MLflow 3.x registries, OpenMetadata extracts model signatures (input/output schema) from run tags when
available, with fallback parsing for older signature formats. Model registry listings are also paginated
automatically, so registries with a large number of versions ingest without truncation.
