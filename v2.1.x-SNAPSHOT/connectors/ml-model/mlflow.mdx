...
- [Troubleshooting](/v2.1.x-SNAPSHOT/connectors/ml-model/mlflow/troubleshooting)
...
For MLflow 3.x registries, OpenMetadata reads model signatures (input schema only -- output schema is not ingested) from the model's
...
To ingest metadata from MLflow, you need to create a service connection. The service connects MLflow with OpenMetadata. Once you create a service, OpenMetadata automatically starts ingesting metadata.
...
![Select Service](/public/images/connector2.0/metadata-ingestion/MLModel/select-service/mlflow.png)
...
![Add New Service Name](/public/images/connector2.0/metadata-ingestion/MLModel/service-name/mlflow.png)
...
![Configure Service Connection](/public/images/connector2.0/metadata-ingestion/MLModel/connection-details/mlflow.png)

- **Tracking URI**: MLflow experiment tracking URI, for example `http://localhost:5000`.
- **Registry URI**: MLflow model registry backend, for example `mysql+pymysql://mlflow:password@localhost:3307/experiments`.
