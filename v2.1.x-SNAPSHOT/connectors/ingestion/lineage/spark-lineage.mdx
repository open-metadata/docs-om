...
- An OpenMetadata **bot JWT token** for authentication. See [Generate Token](/v2.1.x-SNAPSHOT/deployment/security/enable-jwt-tokens#generate-token) for how to generate one.
...
Authenticate with an OpenMetadata bot JWT: set `spark.openlineage.transport.auth.type` to `api_key` and `spark.openlineage.transport.auth.apiKey` to your token. See [Generate Token](/v2.1.x-SNAPSHOT/deployment/security/enable-jwt-tokens#generate-token).
