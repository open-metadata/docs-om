...
Start with **Allow service principals to use Power BI APIs**, which lets the service principal call the Power BI REST APIs. The three admin API settings below then control what the [PowerBI admin APIs](/v2.1.x-SNAPSHOT/connectors/dashboard/powerbi#powerbi-admin-and-non-admin-apis) return, and each one adds a layer of detail to the same [GetScanResult](https://learn.microsoft.com/en-us/rest/api/power-bi/admin/workspace-info-get-scan-result) response that the connector reads.
