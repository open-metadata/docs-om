...
- [Troubleshooting](/v2.1.x-SNAPSHOT/connectors/dashboard/powerbi/troubleshooting)
...
Log in to the [Power BI](https://app.powerbi.com/) as Admin and enable the tenant settings below from [Tenant settings](https://app.powerbi.com/admin-portal/tenantSettings?experience=power-bi) in the Admin portal.
Start with **Allow service principals to use Power BI APIs**, which lets the service principal call the Power BI REST APIs. Of the three settings in the table below, the first authorizes the service principal to call the [PowerBI admin APIs](/v2.1.x-SNAPSHOT/connectors/dashboard/powerbi#powerbi-admin-and-non-admin-apis). The other two add detail to the [GetScanResult](https://learn.microsoft.com/en-us/rest/api/power-bi/admin/workspace-info-get-scan-result) response that the connector reads.

| Tenant setting | What it unlocks | If disabled |
|---|---|---|
| **Service principals can access read-only admin APIs** | Lets an app registered in Microsoft Entra ID authenticate to the read-only admin APIs with a service principal instead of user credentials. This is the base requirement for the `GetScanResult` API that supplies dashboard, chart, and datamodel metadata. See [Enable service principal authentication for read-only admin APIs](https://learn.microsoft.com/en-us/fabric/admin/enable-service-principal-admin-apis). | No metadata is ingested. |
| **Enhance admin APIs responses with detailed metadata** | Adds semantic model table and column names to the `GetScanResult` response, which OpenMetadata ingests as datamodel tables and columns. See [Enabling enhanced metadata scanning](https://learn.microsoft.com/en-us/fabric/governance/metadata-scanning-overview#enabling-enhanced-metadata-scanning). | Data models are ingested without their tables and columns. |
| **Enhance admin APIs responses with DAX and mashup expressions** | Adds table-level and column-level DAX and mashup expressions to the `GetScanResult` response. OpenMetadata parses these expressions to enrich datamodel metadata and to resolve lineage back to source tables and columns. | Lineage is incomplete, because the expressions that link data models to their source tables and columns are missing. |
...
Go to the `Azure Ad app registrations` page, select your app and add the permissions below to the app for the PowerBI service, and grant admin consent for the same.
...
To create a workspace, see [Create a workspace in Power BI](https://learn.microsoft.com/en-us/power-bi/collaborate-share/service-create-the-new-workspaces). For service principal requirements and limitations, see [Embed Power BI content with service principal and an application secret](https://learn.microsoft.com/en-us/power-bi/developer/embedded/embed-service-principal#considerations-and-limitations).
...
- **Use Admin APIs**: Option for using the PowerBI admin APIs. Refer to the [Power BI Admin and Non-Admin APIs](/v2.1.x-SNAPSHOT/connectors/dashboard/powerbi#powerbi-admin-and-non-admin-apis) section for more information.
