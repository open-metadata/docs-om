...
Follow the [Docker Compose installation instructions](https://docs.docker.com/compose/install/linux/) to install Docker Compose version 2.0.0.
...
```commandline
curl -sL -o docker-compose.yml https://github.com/open-metadata/OpenMetadata/releases/download/2.0.2-release/docker-compose.yml

curl -sL -o docker-compose-postgres.yml https://github.com/open-metadata/OpenMetadata/releases/download/2.0.2-release/docker-compose-postgres.yml
```

```commandline
wget https://github.com/open-metadata/OpenMetadata/releases/download/2.0.2-release/docker-compose.yml

wget https://github.com/open-metadata/OpenMetadata/releases/download/2.0.2-release/docker-compose-postgres.yml
```
...
<Warning>
The `docker-compose.yml` asset for this release still defaults to the `2.0.0` database, server, and ingestion images. Set the environment variables below before starting the stack, or you'll get `2.0.0` instead of the `2.0.2` security and maintenance fixes.
</Warning>

```commandline
export OPENMETADATA_DB_IMAGE=docker.getcollate.io/openmetadata/db:2.0.2
export OPENMETADATA_SERVER_IMAGE=docker.getcollate.io/openmetadata/server:2.0.2
export OPENMETADATA_INGESTION_IMAGE=docker.getcollate.io/openmetadata/ingestion:2.0.2
...
470cc8149826   openmetadata/server:2.0.2                             "./openmetadata-star…"   45 seconds ago   Up 43 seconds             3306/tcp, 9200/tcp, 9300/tcp, 0.0.0.0:8585-8586->8585-8586/tcp   openmetadata_server
63578aacbff5   openmetadata/ingestion:2.0.2                           "./ingestion_depende…"   45 seconds ago   Up 43 seconds             0.0.0.0:8080->8080/tcp                                           openmetadata_ingestion
...
08947ab3424b   openmetadata/db:2.0.2                                  "/entrypoint.sh mysq…"   45 seconds ago   Up 44 seconds (healthy)   3306/tcp, 33060-33061/tcp                                        openmetadata_mysql
```
...
Access OpenMetadata at `http://localhost:8585`. Use the following credentials to log in to OpenMetadata.
...
Access Airflow at `http://localhost:8080`. Use the following credentials to log in to Airflow.
- Username: `admin`
- Password: `admin`
...
Please follow our [Enable Security Guide](/v2.1.x-SNAPSHOT/deployment/docker/security) to configure security for your OpenMetadata
installation.

## Next Steps

1. Refer the [How-to Guides](/v2.1.x-SNAPSHOT/how-to-guides) for an overview of all the features in OpenMetadata.
2. Visit the [Connectors](/v2.1.x-SNAPSHOT/connectors) documentation to see what services you can integrate with
   OpenMetadata.
