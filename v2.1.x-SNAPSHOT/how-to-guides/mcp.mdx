...
Connect using your existing OpenMetadata login - the same way you sign in to the OpenMetadata UI. No tokens to generate or manage. OAuth works with all supported SSO providers (Google, Azure, Okta, Auth0, LDAP, SAML, and more). This flow requires a browser login, so it's best suited to clients a person runs interactively (Claude Desktop, Cursor, VS Code). For unattended agents or automation, use a Bot's [Personal Access Token](#personal-access-token-pat) instead.

Learn more about [OAuth 2.0 Authentication](/v2.1.x-SNAPSHOT/how-to-guides/mcp/oauth).

### Personal Access Token (PAT)

For unattended agents, automations, or environments where browser-based login is not available, use a Personal Access Token generated from a **Bot** rather than a personal user account. A Bot is a service-account identity built for this purpose: it isn't tied to a specific person, and it keeps agent access auditable and separate from any individual's login. See [How to Set Up Bots](/v2.1.x-SNAPSHOT/developers/bots) to create one and assign it the role-based access policy your agent needs.

If you only need a quick, personal test connection tied to your own account:

1. Go to `<YOUR-OpenMetadata-SERVER>/users/<YOUR-USERNAME>/access-token` and select **Generate New Token**. This gives your AI agent the same role and access policy assigned to you in OpenMetadata. For any recurring or production agent use case, create a Bot instead so the agent's access isn't tied to your personal account.
