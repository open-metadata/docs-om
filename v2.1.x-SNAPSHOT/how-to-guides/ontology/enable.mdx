...
**RDF Inference Materialization** (`RdfInferenceApp`) is the second application RDF registers. It materializes enabled inference rules into their durable named graphs on a schedule — **every 5 minutes** by default — and only rebuilds rules whose dirty flag is set.

| Setting | Default | Purpose |
|---|---|---|
| `force` | `false` | Re-materialize every enabled rule, even when its dirty flag is clear. |
| `ruleName` | — | Run one named rule on demand instead of the whole set. |
