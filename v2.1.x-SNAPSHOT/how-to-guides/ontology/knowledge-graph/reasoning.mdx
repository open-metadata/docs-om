...
- **Configuration** — `force: true` re-materializes every enabled rule even when its dirty flag is clear; `ruleName` runs one rule on demand.

Trigger it by hand when you have just changed a rule and do not want to wait:

```bash
curl -X POST "$OM_HOST/api/v1/apps/trigger/RdfInferenceApp" \
  -H "Authorization: Bearer $ADMIN_TOKEN"
```

`POST /api/v1/rdf/rules/materialize` does the same thing through the RDF API.
