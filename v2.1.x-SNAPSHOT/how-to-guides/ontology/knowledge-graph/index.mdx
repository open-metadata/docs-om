...
The **Knowledge Graph** is OpenMetadata's entire catalog — every table, column, dashboard, pipeline, ML model, glossary term, tag, owner, test case, data product, and lineage edge — represented as one connected RDF graph, addressable by stable IRIs and queryable with SPARQL.
...
An LLM agent working against a catalog has three ways to find things, and they are not equally good:
