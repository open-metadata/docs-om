...
| `RdfInferenceApp` — **RDF Inference Materialization** | Materializes enabled inference rules into their durable per-rule named graphs. Runs on a schedule (**every 5 minutes** by default). Config: `force` (default `false`) to re-materialize every enabled rule even when its dirty flag is clear, `ruleName` for an on-demand single-rule run. |
...
# Materialize inference rules now, instead of waiting for the schedule
curl -X POST "$OM_HOST/api/v1/apps/trigger/RdfInferenceApp" -H "Authorization: Bearer $TOKEN"
```
