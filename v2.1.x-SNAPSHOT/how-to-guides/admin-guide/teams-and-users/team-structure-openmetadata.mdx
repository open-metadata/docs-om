...
- Only `Group` teams can have `Users` as direct members. `Organization`, `Business Unit`, `Division`, and `Department` teams cannot take Users directly; add users to a `Group` team nested under them instead.
</Warning>
