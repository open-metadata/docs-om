...
OpenMetadata imports and exports data contracts in the [Open Data Contract Standard (ODCS)](https://bitol-io.github.io/open-data-contract-standard/latest/) format. An import converts the ODCS document into an OpenMetadata data contract on one table. It doesn't store the original file.
...
| `servers` | Not imported. OpenMetadata takes connection details from the table's service. |
...
| `authoritativeDefinitions`, `transformSourceObjects` | Kept for export only. |
| `tags` | Not imported. Tag the table's columns in OpenMetadata to classify them or link glossary terms. |
