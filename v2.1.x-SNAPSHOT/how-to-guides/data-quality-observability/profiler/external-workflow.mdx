...
Note that this requires OpenMetadata 1.13 or higher.
...
```commandline
pip install "openmetadata-ingestion[<connector>,datalake,trino]~=2.0.0.0"
```

Where `<connector>` is the name of the connector that you want to run against. Each specific installation command
will be shared on its documentation [page](/v2.1.x-SNAPSHOT/connectors/database).

For example, to run against Athena, we need to install:

```commandline
pip install "openmetadata-ingestion[athena,datalake,trino]~=2.0.0.0"
```
...
You can get details of how to define a yaml file for data profiler for each connector [here](/v2.1.x-SNAPSHOT/connectors/database).
