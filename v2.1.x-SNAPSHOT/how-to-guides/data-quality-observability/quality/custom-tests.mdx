...
Once you have created the package, add the logic in your `<yourTest>.py` file. Create a class named `<YourTest>Validator` that inherits from `BaseTestValidator`. You can also inherit from `SQAValidatorMixin` to access additional methods. Implement the `run_validation` method to return a `TestCaseResult` object. The following skeleton shows the validator structure.
