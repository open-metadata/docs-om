...
| `entities` | array of string | No | Read several entities in one call instead of `entityType`/`fqn`. Each item is `entityType:fqn`, e.g. `table:sample_data.ecommerce_db.shopify.dim_address`. Maximum 10 per call. |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `entityType` | string | Yes | Registered entity type to create, e.g. `glossary`, `glossaryTerm`, `classification`, `tag`, `domain`, `dataProduct`, `metric`, or `page` for a Knowledge Center article. |
...
| `owners` | array of string | No | User or team names to own the entity, e.g. `["john.doe", "Data Platform"]`. A name that resolves to neither fails the call. |
| `tags` | array of string | No | Tag FQNs to apply, e.g. `["PII.Sensitive", "Tier.Tier1"]`. |
