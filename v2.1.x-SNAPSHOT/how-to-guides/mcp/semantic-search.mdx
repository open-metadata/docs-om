...
The Semantic Search tool extends OpenMetadata's [MCP Server](/v2.1.x-SNAPSHOT/how-to-guides/mcp) with the ability to
...
configuration](/v2.1.x-SNAPSHOT/deployment/semantic-search) before it can be used through MCP.
...
- Semantic Search [enabled and configured](/v2.1.x-SNAPSHOT/deployment/semantic-search) in your deployment
- The [MCP Application installed](/v2.1.x-SNAPSHOT/how-to-guides/mcp#installing-mcp-server) in OpenMetadata
- A [Personal Access Token](/v2.1.x-SNAPSHOT/how-to-guides/mcp#personal-access-token-pat)
  for authentication
...
| Configuration | Available by default | Requires [enabling](/v2.1.x-SNAPSHOT/deployment/semantic-search) and an embedding provider |
...
- Ensure entities have been embedded by running a Reindex from the OpenMetadata UI. See the
  [deployment guide](/v2.1.x-SNAPSHOT/deployment/semantic-search#generating-embeddings) for details.
...
### No results returned
- Check that the `dataAssetEmbeddings` index (or `<clusterAlias>_dataAssetEmbeddings` if you've set a cluster alias) exists in your search backend and contains documents.
