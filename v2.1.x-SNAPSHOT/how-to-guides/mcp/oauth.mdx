...
OAuth 2.0 is **recommended for interactive clients** (Claude Desktop, Cursor, VS Code, and similar tools a person signs into) because it removes token management for the human user. It requires a one-time browser login and cannot complete without user interaction, so it doesn't work for headless or unattended processes. For an automated agent or long-running unattended process, use a **Bot's Personal Access Token** instead (see [Personal Access Token (PAT)](/v2.1.x-SNAPSHOT/how-to-guides/mcp#personal-access-token-pat) and [How to Set Up Bots](/v2.1.x-SNAPSHOT/developers/bots)).
...
<CardGroup cols={2}>
  <Card title="Google SSO" href="/v2.1.x-SNAPSHOT/deployment/security/google">
    Sign in with your Google Workspace account.
  </Card>
  <Card title="Azure AD SSO" href="/v2.1.x-SNAPSHOT/deployment/security/azure">
    Sign in with your Microsoft / Azure AD account.
  </Card>
  <Card title="Okta SSO" href="/v2.1.x-SNAPSHOT/deployment/security/okta">
    Sign in with your Okta account.
  </Card>
  <Card title="Auth0 SSO" href="/v2.1.x-SNAPSHOT/deployment/security/auth0">
    Sign in with your Auth0 account.
  </Card>
  <Card title="Amazon Cognito" href="/v2.1.x-SNAPSHOT/deployment/security/amazon-cognito-sso">
    Sign in with Amazon Cognito.
  </Card>
  <Card title="Custom OIDC" href="/v2.1.x-SNAPSHOT/deployment/security/custom-oidc">
    Sign in with any OIDC-compatible provider.
  </Card>
  <Card title="SAML" href="/v2.1.x-SNAPSHOT/deployment/security/saml">
    Sign in with your SAML identity provider.
  </Card>
  <Card title="LDAP" href="/v2.1.x-SNAPSHOT/deployment/security/ldap">
...
**Session token expiry**: If a client authenticates through this login form (or LDAP/SAML) rather than through the OAuth flow described above, the resulting session token expires after **1 hour by default**, configurable by an admin under **Settings > Preferences > Login Configuration**. A long-running or headless agent using this login method must detect a 401 response and re-authenticate, which requires a human to sign in again. For a process that needs to run unattended for more than an hour, use a Bot's [Personal Access Token](/v2.1.x-SNAPSHOT/how-to-guides/mcp#personal-access-token-pat) instead, which isn't subject to this 1-hour limit.
...
For detailed instructions on configuring each SSO provider, see the [Security documentation](/v2.1.x-SNAPSHOT/deployment/security).
...
<CardGroup cols={2}>
  <Card title="Claude Desktop" href="/v2.1.x-SNAPSHOT/how-to-guides/mcp/claude">
    Connect via Anthropic's AI assistant.
  </Card>
  <Card title="Cursor" href="/v2.1.x-SNAPSHOT/how-to-guides/mcp/cursor">
    Connect via Cursor IDE.
  </Card>
  <Card title="VS Code" href="/v2.1.x-SNAPSHOT/how-to-guides/mcp/vscode">
    Connect via Visual Studio Code.
  </Card>
  <Card title="Claude Code" href="/v2.1.x-SNAPSHOT/how-to-guides/mcp/claude-code">
    Connect via Claude Code CLI.
  </Card>
  <Card title="Goose" href="/v2.1.x-SNAPSHOT/how-to-guides/mcp/goose">
