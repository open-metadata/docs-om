...
- **OAuth 2.0 (recommended)**: Sign in with your existing OpenMetadata login. See [OAuth 2.0 Authentication](/v2.1.x-SNAPSHOT/how-to-guides/mcp/oauth) for setup instructions.
- **Personal Access Token (PAT)**: For environments where browser-based login isn't available. Generate a token as described in [Personal Access Token (PAT)](/v2.1.x-SNAPSHOT/how-to-guides/mcp#personal-access-token-pat), then include it in the Authorization header:
```http
