...
By default, an asset can only belong to one Data Product, and any Data Product added to an asset must belong to the same domain as the asset itself. Admins can change this in [Data Asset Rules](/v2.1.x-SNAPSHOT/how-to-guides/admin-guide/data-asset-rules) using the **Multiple Data Products are not allowed** and **Data Product Domain Validation** rules.

## Troubleshooting

Common issues when adding assets to Data Products:

### Data Product Domain Validation Doesn't Recognize Subdomains

**Problem**: An asset assigned directly to a subdomain (for example, `customer_domain.finance`) can't be added to a Data Product assigned to the subdomain's parent domain (`customer_domain`), even though the subdomain is nested under it. The same asset assigned directly to the parent domain can be added without issue.

**Cause**: The **Data Product Domain Validation** rule currently checks for an exact domain match between an asset and a Data Product. It doesn't yet treat a subdomain as a match for its parent domain.

**Status**: Disabling the rule in [Data Asset Rules](/v2.1.x-SNAPSHOT/how-to-guides/admin-guide/data-asset-rules) doesn't currently resolve this for subdomains, since the underlying domain-matching logic needs to recognize the parent-subdomain relationship independently of the toggle. This is a known limitation the OpenMetadata team is actively addressing.
...
<CardGroup cols={1}>
<Card title="How to Use Domains" href="/v2.1.x-SNAPSHOT/how-to-guides/data-marketplace/domains">
  Learn about Domains.
</Card>
</CardGroup>
