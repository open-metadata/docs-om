...
[BaseWorkflow](https://github.com/open-metadata/OpenMetadata/blob/main/ingestion/src/metadata/workflow/base.py) is the orchestration base that runs components in order.

A workflow consists of [Source](/v2.1.x-SNAPSHOT/api-reference/sdk/python/build-connector/source) and [Sink](/v2.1.x-SNAPSHOT/api-reference/sdk/python/build-connector/sink). It also provides support for [Stage](/v2.1.x-SNAPSHOT/api-reference/sdk/python/build-connector/stage) and [BulkSink](/v2.1.x-SNAPSHOT/api-reference/sdk/python/build-connector/bulk-sink).
...
<CardGroup cols={2}>
<Card title="Source" href="/v2.1.x-SNAPSHOT/api-reference/sdk/python/build-connector/source" >
    The connector to external systems which outputs a record for downstream to process.
  </Card>
  <Card title="Sink" href="/v2.1.x-SNAPSHOT/api-reference/sdk/python/build-connector/sink" >
    It will get the event emitted by the source, one at a time.
  </Card>
  <Card title="Stage" href="/v2.1.x-SNAPSHOT/api-reference/sdk/python/build-connector/stage" >
    It can be used to store the records or to aggregate the work done by a processor.
  </Card>
  <Card title="BulkSink" href="/v2.1.x-SNAPSHOT/api-reference/sdk/python/build-connector/bulk-sink" >
    It can be used to bulk update the records generated in a workflow.
  </Card>
</CardGroup>

Read more about Workflow management [here](https://github.com/open-metadata/OpenMetadata/blob/main/ingestion/src/metadata/workflow/README.md).
