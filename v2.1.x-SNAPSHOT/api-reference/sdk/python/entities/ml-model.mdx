...
While we can already extract certain pieces of information automatically via our Connectors (e.g., [Mlflow](/v2.1.x-SNAPSHOT/connectors/ml-model/mlflow)), there are attributes that we'll need to fill in by ourselves. Thanks to the [Solution Design](/v2.1.x-SNAPSHOT/main-concepts/high-level-design) of OpenMetadata and the [Python SDK](/v2.1.x-SNAPSHOT/api-reference/sdk/python), this is going to be a rather easy task that will unlock the full power of your **organization's metadata**.
