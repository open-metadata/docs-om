...
<ParamField query="status" type="string">
  Filter by contract status: `Draft`, `In Review`, `Approved`, `Rejected`, `Deprecated`, or `Archived`.
</ParamField>
...
contracts = DataContracts.list(params={"status": "Approved"})
...
curl "{base_url}/api/v1/dataContracts?status=Approved&fields=owners" \
  -H "Authorization: Bearer {access_token}"
