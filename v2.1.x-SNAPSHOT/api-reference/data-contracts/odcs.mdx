...
`PUT /v1/dataContracts` (the non-ODCS endpoint) can't clear these passthrough fields—a plain PUT that changes
something else leaves them untouched. To clear them, use `PUT /odcs?mode=replace` with a document that omits
them, or a PATCH that explicitly removes them.
