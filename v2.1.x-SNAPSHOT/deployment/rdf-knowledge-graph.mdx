---
title: Enable RDF (Knowledge Graph) (Beta) | OpenMetadata Deployment Guide
...
sidebarTitle: Enable RDF (Knowledge Graph) (Beta)
---
...
This page covers **deploying** the triple store. For what the knowledge graph is, what you can do with it, and how
Ontology Studio uses it, see [Ontology & Knowledge Graph](/v2.1.x-SNAPSHOT/how-to-guides/ontology).
...
RDF is configured in `openmetadata.yaml` under the `rdf` section. Every setting can be overridden with an environment variable.
...
| `RDF_STORAGE_TYPE` | `FUSEKI` | Triplestore backend. `FUSEKI` is the only implemented option (see warning below). |
| `RDF_ENDPOINT` | `http://localhost:3030/openmetadata` | The full URL OpenMetadata uses to reach Fuseki, ending in the dataset name. `RDF_REMOTE_ENDPOINT` is a deprecated fallback used only when this is unset. |
...
| `RDF_BASE_URI` | `https://open-metadata.org/` | Base URI used when minting RDF resource IRIs. |
...
| `RDF_WRITE_MAX_RETRIES` | `2` | Retries for idempotent write requests after the initial attempt. |
...
| `RDF_BULK_LINEAGE_EDGE_BATCH_SIZE` | `50` | Detailed lineage edges per SPARQL update. |
...
| `RDF_DEFAULT_INFERENCE_LEVEL` | `NONE` | Default reasoning level: `NONE`, `RDFS`, `OWL_LITE`, `OWL_DL`, or `CUSTOM`. `CUSTOM` applies OpenMetadata's own rules — transitive lineage, tag propagation, domain inheritance. |
| `RDF_MATERIALIZED_INFERENCE_ENABLED` | `false` | Use durable per-rule inferred named graphs produced inside the triplestore instead of building an in-memory Jena model. **Recommended for anything past a small catalog.** |
| `RDF_MAX_IN_MEMORY_INFERENCE_TRIPLES` | `100000` | Above this store size, in-process inference falls back to direct execution — with a `warning` in the response, and **without** inference applied. |
| `RDF_CACHE_INFERRED_TRIPLES` | `false` | Cache bounded in-memory inference models for 60 seconds. |

### Validation and OWL profile

| Environment Variable | Default | Description |
|---|---|---|
| `RDF_SHACL_VALIDATION_MODE` | `REPORT` | `OFF`, `REPORT`, or `ENFORCE_IMPORTS`. Never blocks entity writes; `ENFORCE_IMPORTS` additionally rejects ontology imports that fail validation. |
| `RDF_STRICT_OWL_PROFILE` | `true` | Reject authored axioms outside the supported OWL 2 DL profile. Keeps reasoning decidable. |

### Linked data, AI, and federation

| Environment Variable | Default | Description |
|---|---|---|
| `RDF_DEREFERENCEABLE_IRIS` | `false` | Expose authenticated, content-negotiated redirects for OpenMetadata-minted IRIs at `/api/v1/lod/entity/...`. |
| `RDF_ASK_COLLATE_ENABLED` | `false` | Expose the optional AI-assisted Ontology Studio flows. Manual authoring is unaffected. |
| `RDF_FEDERATION_ENABLED` | `false` | Master switch for federated SPARQL (`SERVICE` clauses to external endpoints). |
| `RDF_FEDERATION_ALLOWED_ENDPOINTS` | `[]` | External SPARQL endpoint URIs permitted in `SERVICE` clauses. Compared **verbatim** — trailing slashes matter. |
...
rdf:
  ...
  cacheInferredTriples: ${RDF_CACHE_INFERRED_TRIPLES:-false}
...
**Federation is a data-exfiltration surface.** A `SERVICE` clause sends part of your query -- and potentially your data -- to a third-party
endpoint. It is disabled by default and the allowlist is empty. Enable it only for endpoints you control or explicitly trust.
...
configuration key and have no effect. Only the environment variables in the tables above are read by the server. If
