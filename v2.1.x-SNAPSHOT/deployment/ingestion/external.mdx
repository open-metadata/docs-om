...
<Tip>
You will find examples of all the workflow's YAML files at each Connector [page](/v2.1.x-SNAPSHOT/connectors).
</Tip>
...
To enable JWT Tokens, you will get more details [here](/v2.1.x-SNAPSHOT/deployment/security/enable-jwt-tokens).

You can refer to the JWT Troubleshooting section [link](/v2.1.x-SNAPSHOT/deployment/security/jwt-troubleshooting) for any issues in
...
or externally, if you have configured any [Secrets Manager](/v2.1.x-SNAPSHOT/deployment/secrets-manager).
...
If you have configured any [Secrets Manager](/v2.1.x-SNAPSHOT/deployment/secrets-manager), you need to let the Ingestion Framework know
how to retrieve the credentials securely.

Follow the [docs](/v2.1.x-SNAPSHOT/deployment/secrets-manager) to configure the secret retrieval based on your environment.

**SSL Configuration**

If you have added SSL to the [OpenMetadata server](/v2.1.x-SNAPSHOT/deployment/security/enable-ssl), then you will need to handle
...
Find more information on how to troubleshoot SSL issues [here](/v2.1.x-SNAPSHOT/deployment/security/enable-ssl/ssl-troubleshooting).
...
If you are using the [Secrets Manager](/v2.1.x-SNAPSHOT/deployment/secrets-manager), you can let the Ingestion client to pick up
...
<CardGroup cols={2}>
<Card title="Airflow" href="/v2.1.x-SNAPSHOT/deployment/ingestion/external/airflow" >
  ...
  <Card title="GitHub Actions" href="/v2.1.x-SNAPSHOT/deployment/ingestion/external/github-actions" >
    Run the ingestion process externally from GitHub Actions
  </Card>
</CardGroup>

<Info>
For code examples running Metadata, Lineage, Usage, Profiler, and Data Quality workflows externally, see [Workflow Examples](/v2.1.x-SNAPSHOT/deployment/ingestion/external/examples).
</Info>
