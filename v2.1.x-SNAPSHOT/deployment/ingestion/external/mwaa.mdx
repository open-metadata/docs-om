...
Where `x.y.z` is the version of the OpenMetadata ingestion package. Note that the version needs to match the server version. If we are using the server at 2.0.2, then the ingestion package needs to also be 2.0.2.0.
...
- The required image is `docker.open-metadata.org/openmetadata/ingestion-base:x.y.z`
  - The same logic as above applies. The `x.y.z` version needs to match the server version. For example, `docker.open-metadata.org/openmetadata/ingestion-base:2.0.2`
