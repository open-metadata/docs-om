...
OpenMetadata integrates with Airflow to orchestrate ingestion workflows. You can use Airflow to [extract metadata](/v2.1.x-SNAPSHOT/connectors/pipeline/airflow) and [deploy workflows] (/deployment/ingestion/openmetadata) directly. This guide explains how to run ingestion workflows in Airflow using three different operators:

1. [Python Operator](#using-the-python-operator)
2. [Docker Operator](/v2.1.x-SNAPSHOT/deployment/ingestion/external/airflow-docker-virtualenv#using-the-docker-operator)
3. [Python Virtualenv Operator](/v2.1.x-SNAPSHOT/deployment/ingestion/external/airflow-docker-virtualenv#using-the-python-virtualenv-operator)
...
-Replace x.y.z with the OpenMetadata version matching your server (e.g., 2.0.0.0).

### Example
```
pip3 install openmetadata-ingestion[mysql,snowflake,s3]==2.0.0.0
```
### Example DAG
...
<Card title="Docker & Virtualenv Operators" href="/v2.1.x-SNAPSHOT/deployment/ingestion/external/airflow-docker-virtualenv">
