...
<CardGroup cols={2}>
  ...
  <Card title="Kubernetes Native" href="/v2.1.x-SNAPSHOT/deployment/ingestion/kubernetes" icon="cube">
...
- The `airflow` extra installs Apache Airflow 3.3.0. Earlier Airflow 2.x versions are no longer supported — upgrade to Airflow 3.3.0 before installing the ingestion modules on an existing Airflow host.
</Tip>
...
And then run the DAG as explained in each [Connector](/v2.1.x-SNAPSHOT/connectors), where `x.y.z` is the same version of your
...
You can check the [Connector Modules](/v2.1.x-SNAPSHOT/connectors) guide above to learn how to install the `openmetadata-ingestion` package with the
...
<Info>
For installation validation, Git Sync guidance, SSL configuration, and troubleshooting Airflow pipeline issues, see the [Airflow Troubleshooting & Advanced](/v2.1.x-SNAPSHOT/deployment/ingestion/openmetadata/troubleshooting) guide.
</Info>
