...
Whether to derive roles from the authentication provider. Roles must be included in the identity provider's token and must match existing OpenMetadata role names. Roles sync on every login, not just the first, so a role change at the provider takes effect the next time the user signs in. If the token omits the roles claim entirely, OpenMetadata leaves existing role assignments untouched. This can promote a user to Admin but never demotes an existing admin assigned through `adminPrincipals`.
