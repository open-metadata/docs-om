...
The sampler module lives at `ingestion/src/metadata/sampler/`. It provides a **unified interface for sampling data** from tables across different database backends. Both the [profiler](/v2.1.x-SNAPSHOT/developers/contribute/codebase-deep-dives/profiler) and [data quality](/v2.1.x-SNAPSHOT/developers/contribute/codebase-deep-dives/data-quality) modules depend on the sampler to get a representative subset of rows for computing metrics and running tests.
